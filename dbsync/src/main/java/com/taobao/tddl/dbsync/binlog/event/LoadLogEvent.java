 ...
 * n            An array of num_fields integers representing the length of each field in the query
                (n is from the Post-Header)

 * n string     An array of n null-terminated strings, each representing a field in the query.
                (The trailing zero is redundant, since the length are stored in the num_fields array.)
                The total length of all strings equals to the sum of all field_lens, plus num_fields bytes for all the trailing zeros.
 * string       table_name,null-terminated string of length table_len+1 bytes
                (The trailing zero is actually redundant since the table_len is known from Post-Header.)
 * string       db, null-terminated string of length db_len+1 bytes</td>
                (The trailing zero is actually redundant since the db_len is known from Post-Header.)
 * string       file_name, variable length string without trailing zero, extending to the end of the event
 *              (determined by the length field of the Common-Header)
...
public class LoadLogEvent extends LogEvent {
    ...
    protected final void copyLogEvent(LogBuffer buffer, final int bodyOffset, FormatDescriptionLogEvent descriptionEvent) {
        /* this is the beginning of the post-header 忽略的slave proxy id */
