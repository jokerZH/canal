 ...
 * 4(unsigned)  slave_proxy_id,
                ...
                distinguish temporary tables that belong to different clients

 * 4(unsigned)  exec_time
                The time from when the query started to when it was logged in the binlog in seconds

 * 1            db_len
                The length of the name of the currently selected database.</td>
 * 2(unsigned)  error_code
                Error code generated by the master. If the master fails, the slave will
                fail with the same error code, except for the error codes
                    ER_DB_CREATE_EXISTS == 1007 and ER_DB_DROP_EXISTS == 1008
 * 2(unsigned)  status_vars_len
                The length of the status_vars block of the Body, in bytes.
                See query_log_event_status_vars "below"
 *
 *              Body for Query_log_event
 *
 * Bytes                        desc
 * -----                        ----
 * status_vars_len bytes        query_log_event_status_vars status_vars
                                Zero or more status variables. Each status variable consists of one byte
                                identifying the variable stored, followed by the value of the variable. The
                                possible variables are listed separately in the table Table_query_log_event_status_vars "below".
                                MySQL always writes events in the order defined below;
                                however, it is capable of reading them in any order.

 * db_len+1                     db
                                The currently selected database, as a null-terminated string.

 * string without trailing zero query
 ...
 *
 *
 * auto_increment   Q_AUTO_INCREMENT == 3       two 2 byte unsigned integers, totally 2+2=4 bytes
 ...
 * The three variables character_set_client, collation_connection, and
 ...
 * The time_zone of the master. See also "System Variables" and
 * "MySQL Server Time Zone Support" in the MySQL manual. This field is written
 * if the length of the time zone string is > 0; otherwise, it is not written.
 *
 *
 *
 * catalog          Q_CATALOG_NZ_CODE == 6      Variable-length string:
 *                                              the length in bytes (1 byte) followed by the
 ...
 * one catalog, "std". This field is written if the length of the catalog is > 0;
 * otherwise it is not written.</td>
 *
 *
 * lc_time_names_number     Q_LC_TIME_NAMES_CODE == 7   2 byte integer
 * A code identifying a table of month and day names. The mapping from codes
 * to languages is defined in sql_locale.cc. This field is written if it is not
 * 0, i.e., if the locale is not en_US.
 *
 *
 * charset_database_number  Q_CHARSET_DATABASE_CODE == 8    2 byte integer
 * The value of the collation_database system variable (in the source code
 ...
 * written if it is not 0.
 *
 *
 *
 *
 * table_map_for_update     Q_TABLE_MAP_FOR_UPDATE_CODE == 9    8 byte integer
 * The value of the table map that is to be updated by the multi-table
...
public class QueryLogEvent extends LogEvent {
    ...
    public static final int MAX_SIZE_LOG_EVENT_STATUS = (  1 + 4 /* type, flags2 */
                                                         ...
                                                         + 1 + 60/* type , user_len , user , host_len , host */);
    /**
     * Fixed data part:
     * <ul>
    ...
    /* 读取Status variables for Query_log_event */
    ...
    public final String getCatalog() { return catalog; }
    ...
    public final long getAutoIncrementIncrement() { return autoIncrementIncrement; }
    public final long getAutoIncrementOffset() { return autoIncrementOffset; }
    public final String getCharsetName() { return charsetName; }
    public final String getTimezone() { return timezone; }
    public final int getClientCharset() { return clientCharset; }
    public final int getClientCollation() { return clientCollation; }
    public final int getServerCollation() { return serverCollation; }
    public final long getSqlMode() { return sql_mode; }
    public final long getFlags2() { return flags2; }
    ...
    public final boolean isAutoIsNull() { return ((flags2 & OPTION_AUTO_IS_NULL) == OPTION_AUTO_IS_NULL); }
    /* Returns the OPTION_NO_FOREIGN_KEY_CHECKS flag. */
    public final boolean isForeignKeyChecks() { return ((flags2 & OPTION_NO_FOREIGN_KEY_CHECKS) != OPTION_NO_FOREIGN_KEY_CHECKS); }
    /* Returns the OPTION_NOT_AUTOCOMMIT flag. */
    public final boolean isAutocommit() { return ((flags2 & OPTION_NOT_AUTOCOMMIT) != OPTION_NOT_AUTOCOMMIT); }
    /* Returns the OPTION_NO_FOREIGN_KEY_CHECKS flag. */
    public final boolean isUniqueChecks() { return ((flags2 & OPTION_RELAXED_UNIQUE_CHECKS) != OPTION_RELAXED_UNIQUE_CHECKS); }
}
